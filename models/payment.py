...
CINETPAY_HEADERS = {'Cinetpay-Version': '2023-04-26'}
...
class PaymentAcquirerCinetpay(models.Model):
    _inherit = 'payment.acquirer'

    provider = fields.Selection(selection_add=[('cinetpay', 'Cinetpay')])
    cinetpay_secret_key = fields.Char(required_if_provider='cinetpay', groups='base.group_user')
    cinetpay_publishable_key = fields.Char(required_if_provider='cinetpay', groups='base.group_user')
    cinetpay_image_url = fields.Char(
        ...
        help="A relative or absolute URL pointing to a square image of your "
             ...
             "https://cinetpay.com/assets/images/common/logo-cinetpay.webp")
    ...
    @api.model
    def _get_cinetpay_api_url(self):
        return 'https://docs.cinetpay.com/'

    @api.model
    def cinetpay_s2s_form_process(self, data):
    ...
    @api.multi
    def cinetpay_s2s_form_validate(self, data):
        self.ensure_one()
    ...
    def _get_feature_support(self):
        ...
        key for the following features:
            ...
            * tokenize: support saving payment data in a payment.tokenize
                        object
        """
        res = super(PaymentAcquirerCinetpay, self)._get_feature_support()
        res['tokenize'].append('stripe')
        return res


class PaymentTransactionCinetpay(models.Model):
    _inherit = 'payment.transaction'

    def _create_cinetpay_charge(self, acquirer_ref=None, tokenid=None, email=None):
        api_url_charge = 'https://%s/charges' % (self.acquirer_id._get_cinetpay_api_url())
        charge_params = {
        ...
        _logger.info('_create_cinetpay_charge: Sending values to URL %s, values:\n%s', api_url_charge, pprint.pformat(charge_params))
        r = requests.post(api_url_charge,
                          auth=(self.acquirer_id.cinetpay_secret_key, ''),
                          params=charge_params,
                          headers=CINETPAY_HEADERS)
        res = r.json()
        _logger.info('_create_cinetpay_charge: Values received:\n%s', pprint.pformat(res))
        return res

    @api.multi
    def cinetpay_s2s_do_transaction(self, **kwargs):
        self.ensure_one()
        result = self._create_cinetpay_charge(acquirer_ref=self.payment_token_id.acquirer_ref, email=self.partner_email)
        return self._cinetpay_s2s_validate_tree(result)


    def _create_cinetpay_refund(self):
        api_url_refund = 'https://%s/refunds' % (self.acquirer_id._get_cinetpay_api_url())
        ...
        _logger.info('_create_cinetpay_refund: Sending values to URL %s, values:\n%s', api_url_refund, pprint.pformat(refund_params))
        r = requests.post(api_url_refund,
                            auth=(self.acquirer_id.cinetpay_secret_key, ''),
                            params=refund_params,
                            headers=CINETPAY_HEADERS)
        res = r.json()
        _logger.info('_create_cinetpay_refund: Values received:\n%s', pprint.pformat(res))
        return res

    @api.multi
    def cinetpay_s2s_do_refund(self, **kwargs):
        self.ensure_one()
        result = self._create_cinetpay_refund()
        return self._cinetpay_s2s_validate_tree(result)
    ...
    def _cinetpay_form_get_tx_from_data(self, data):
        ...
        if not reference:
            cinetpay_error = data.get('error', {}).get('message', '')
            _logger.error('Stripe: invalid reply received from stripe API, looks like '
                          'the transaction failed. (error: %s)', cinetpay_error  or 'n/a')
    ...
    @api.multi
    def _cinetpay_s2s_validate_tree(self, tree):
        self.ensure_one()
        if self.state != 'draft':
    ...
    def _cinetpay_form_validate(self,  data):
        return self._cinetpay_s2s_validate_tree(data)
...
class PaymentTokenStripe(models.Model):
    ...
    @api.model
    def cinetpay_create(self, values):
        token = values.get('cinetpay_token')
        description = None
        ...
        if values.get('cc_number'):
            url_token = 'https://%s/tokens' % payment_acquirer._get_cinetpay_api_url()
            payment_params = {
            ...
            r = requests.post(url_token,
                              auth=(payment_acquirer.cinetpay_secret_key, ''),
        ...
        res = self._cinetpay_create_customer(token, description, payment_acquirer.id)

        # pop credit card info to info sent to create
        for field_name in ["cc_number", "cvc", "cc_holder_name", "cc_expiry", "cc_brand", "cinetpay_token"]:
            values.pop(field_name, None)
        return res


    def _cinetpay_create_customer(self, token, description=None, acquirer_id=None):
        if token.get('error'):
            _logger.error('payment.token.cinetpay_create_customer: Token error:\n%s', pprint.pformat(token['error']))
            raise Exception(token['error']['message'])

        if token['object'] != 'token':
            _logger.error('payment.token.cinetpay_create_customer: Cannot create a customer for object type "%s"', token.get('object'))
            raise Exception('We are unable to process your credit card information.')

        if token['type'] != 'card':
            _logger.error('payment.token.cinetpay_create_customer: Cannot create a customer for token type "%s"', token.get('type'))
            raise Exception('We are unable to process your credit card information.')

        payment_acquirer = self.env['payment.acquirer'].browse(acquirer_id or self.acquirer_id.id)
        url_customer = 'https://%s/customers' % payment_acquirer._get_cinetpay_api_url()
        ...
        if customer.get('error'):
            _logger.error('payment.token.cinetpay_create_customer: Customer error:\n%s', pprint.pformat(customer['error']))
